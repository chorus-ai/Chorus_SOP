...
This SOP is narrowly focused on linking imaging and waveform data acquired on the same patient to ensure data integrity. This SOP **does not** cover any procedures related to management of bundles of related waveform or imaging files that are all acquired in a single procedure. It **does not** cover procedures for deidentification by data-shifiting or otherwise. It **does not** cover the procedures for submitting data to the cloud. It **does not** cover any similar data linking activities that occur in the cloud environment after sites submit data.
...
- **file_id**
  ...
  - The most critical constraints of this column:
    ...
    - IF you are using the *file_id* value as *procedure_occurrence_id*, which we strongly recommend, that id must:
      - Not overlap with other *procedure_occurrence_id* values assigned to EHR-derived procedures
      - Be an integer (or bigint if necessary/preferred)
  ...
  - *DATATYPE*: integer (preferred) or bigint
- **person_id**
...
4. *procedure_concept_id* values should either be set to **4180938** (Imaging Procedure) for images, or **4141651** (Monitoring Procedure) for waveform files
5. *procedure_source_value* values should include a concatenated representation of (1) the target file path/name and (2) the group identifier associated with the particular file, should there be one.
    - You are free to concatenate these values how you prefer, but we ask you document your method and aim for consistency
