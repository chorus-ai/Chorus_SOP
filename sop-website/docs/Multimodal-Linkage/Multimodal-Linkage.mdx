...
This SOP is narrowly focused on linking different modalities of data acquired on the same patient to ensure data integrity. This SOP does not cover any procedures related to management of bundles of related waveform files or imaing files that are all acquired in a single procedure. It does not cover procedures for deidentification by data-shifiting. It does not cover the procedures for submitting data to the cloud. It does not cover any similar data linking activities that occur in the cloud environment after sites submit data.
