...
Waveform data refers to all data acquired from bedside monitors and devices, including alarms, numerics data obtain at regular of irregular intervals, and high-frequency (>1 Hz) waveform data such as digitized EKG tracings. The waveforms should be deidentified.   For Y1 submission, WFDB file format is recommended but not required. 
...
If the file upload is interrupted, and you need to restart, first locate the local journal file (`[journal_path] local_journal` in the `config.toml` file), then call
```
