...
3. The DGS will organize the files using the folder structure as described in the [Local File Organization](#local-file-organization) section below.
...
Waveform data refers to all data acquired from bedside monitors and devices, including alarms, numerics data obtained at regular or irregular intervals, and high-frequency (>1 Hz) waveform data such as digitized EKG tracings. The waveforms should be deidentified.   
...
session_id is a unique identifier that can be chosen by sites to represent a single continuous recording.
...
This virtual environment is hosted in an Azure platform. The containers structure is setup such that each DGS has its own container (e.g. mgh).  The container name for each DGS is used in the `config.toml` files as `[DGS_container]` in the Configuration section.
...
1. The DGS local data shall be organized according to the [Local File Organization](#local-file-organization) section.
...
> **MOVED** Please refer to [Readme.md](https://github.com/chorus-ai/chorus-extract-upload/blob/main/README.md) for installation and usage instructions.
