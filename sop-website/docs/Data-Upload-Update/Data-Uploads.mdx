...
This SOP and the associated tool scans a DGS's data folder structure 
...
## Configuration File:
A `config.toml` file needs to be customized for each size.
...
If you are transferring files from a cloud account to CHoRUS, please refer to you institution's documentation to retrieve credentials for other storage clouds.  For a list of supported authentication mechanisms for each tested cloud providers, please use
```
