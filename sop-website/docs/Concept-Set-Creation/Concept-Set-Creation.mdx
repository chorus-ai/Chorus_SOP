...
- Return to the `Concept Set Expression` section and click the `Optimize` button (if available). This will remove redundant child concepts that are already covered by a parent with Descendants checked, simplifying the expression logic and making it easier to manage.
...
- Enter the Concept Set name in the white input field at the top of the screen. Use the following naming format to ensure consistency across the project:
...
### Step X: Importing Concept Identifiers or Recreating Concept Sets from Another Atlas Instance
To recreate a concept set using known OMOP Concept IDs (e.g., from another Atlas instance or documentation):

1. Go to **`Concept Sets`** → click **`New Concept Set`**.
2. Rename the concept set to follow naming conventions (avoid the default name).
3. Click the **`Import`** tab.
4. Under **`Concept Identifiers`**, paste the Concept IDs (e.g., `132797` for Sepsis). If entering multiple Concept IDs, separate them using commas without spaces.
5. Before clicking `Add To Concept Set`, select:
   - `Descendants` to include child concepts
   - `Exclude` if the concept should be subtracted
   - Both `Exclude` and `Descendants` to exclude entire branches
6. Click **`Add To Concept Set`**.

![Importing Concept Identifiers Example](./images/import-concept-set-example.png)

#### After Importing:
Continue with the standard Concept Set creation workflow:
- Go to **`Concept Set Expression`** to review logic
- Click **`Included Concepts`** to validate the resolved concept list
- Use **`Optimize`** (if available) to simplify the expression
- Enter **Concept Set Description** with Clinical Justification
- Click **`Save`** and register the set in the inventory

> **Tip:** This method is ideal for transferring concept sets across environments or manually reconstructing shared definitions.
