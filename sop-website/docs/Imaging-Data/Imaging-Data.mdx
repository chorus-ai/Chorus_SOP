...
6. Identify accession numbers for specific image tests. The preferred method is using EHR search to filter tests based on encounter (e.g., CDW) and not pulling directly from PACS based on the patient identifiers. This is to simplify image test mapping to the primary encounter of interest for the multimodal data pull.

7. Map header metadata relevant to your local site with header metadata selected by CHORUS (list of metadata fields below). Mapping to OMOP to be completed later. RSNA LOINC codes specifying the type of study would be useful to include. Imaging studies should have a corresponding entry in the PROCEDURE_OCCURENCE OMOP table. Thus imaging studies can be linked to the rest of the data using the procedure_occurence_id field. Refer to multimodal linkage SOP on specifics about linking images to the other data: https://chorus-ai.github.io/Chorus_SOP/docs/Multimodal-Linkage/ . For date shifting, please refer to the  Date Shifting SOP.
...
10. Identifiable cross walk table creation: maps all original DICOM metadatafields to the deidentification procedure (e.g., replacement, erase, etc) and the new field in the deidentified DICOM, so this can be used as a crosswalk (crosswalk table stays local at site and is not shared).

11. Deidentified DICOM metadata table (for sharing): to enable imaging data querying without loading DICOM data.

12. DICOM with metadata deidentified: touse a CHORUS-specific CTP version; DICOM files not to be shared until pixel deidentification solution devised unless site already has an approved solution.
