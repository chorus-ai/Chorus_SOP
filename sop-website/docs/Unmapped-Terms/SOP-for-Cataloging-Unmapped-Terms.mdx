...
| `important_for_research`   | Indicates whether the concept has been identified as important for research purposes (can be 'Yes'/'No'/'Unknown').   | String  | `Yes`   | No  |
| `who_identified_importance`| The name or role of the individual(s) who indicated the term's research importance.   | String   | `Dr. Jane Smith, Oncology Specialist`   | No  |
| `research_question`   | A description of the research questions the concept was deemed important to address.  | String   | `Study of intubation practices and clinical outcomes.` | No   |
| `comments`   | Captures notes on the mapping process, including reasons for unsuccessful mappings, challenges, and the identity of the person responsible. | String | `Concept too granular, no equivalent in standardized vocabulary; Senior Data Analyst, John Doe` | No   |

- Please note that if additional metadata is not available at this time, you may proceed by submitting the data with only the required fields.
...
2. **Prioritization**
Terms will be prioritized using the following criteria:
...
4. **Mapping and Validation by Topic Groups**
The standards team will evaluate whether the terms can be mapped to the OMOP CDM (e.g., using a broad match) and assess the relevance of unmapped concepts for potential inclusion in the critical care flowsheet ontology. All terms will be validated in topic-based batches. Validation includes:
