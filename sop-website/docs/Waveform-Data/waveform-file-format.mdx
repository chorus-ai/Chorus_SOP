...
This document defines acceptable file formats for waveform data contributions in the CHoRUS project. The purpose is to ensure consistent formatting of clinical waveform data. For a high level summary of the process for contributing waveform data to CHoRUS, see the SOP on [contributing waveforms](Waveform-Data.mdx). All waveform data will eventually be converted to WFDB but we are requesting submissions in your preferred file format (ideally along with explicit timestamps) to take advantage of tools which help correct for gaps in an appropriate manner.
...
- `telemetry/`:
  - All telemetry waveforms should be submitted under the telemetry folder. High-frequency waveform signal files and annotation files should be named as `<session_id>` followed by the appropriate extension for a signal file or annotation file in the submitted format. Low-frequency "numerics" signals should be submitted in a CSV file with a "n" before the extension.
- `eeg/`:
  - All EEG waveforms should be submitted under the EEG folder. EEG waveform signal files and annotation files should be named as `<session_id>` followed by the appropriate extension for a signal file or annotation file in the submitted format. Low-frequency "numerics" signals should be submitted in a CSV file with a "n" before the extension.
- `<session_id>/`:
  - Session ID indicates a unique recording for the patient. A single patient (i.e. a single `<person_id>`) may contain multiple sessions.

---
## 3. Preparing Waveforms 

Ultimately, all waveform files will be run through the official **CHoRUS to WFDB converter** to ensure consistent handling of signals. Sites must prepare their waveform data in a manner that makes it possible to be run through this converter. If you are manipulating or converting the format of your waveform data prior to submission, please do so in a manner consistent with the guidelines below. We don't recommend manipulating your waveform data beyond deidentifying it per your site requirements for submission to CHoRUS.
...
As requested below you will need to map each signal from your submitted waveforms to the appropriate **signal label** provided in the links above. 

### 3.5. Low Frequency Signals ("Numerics")
...
We encourage submitting waveforms in your preferred format which is as close as you can get to the output from your monitor. When waveform data is submitted with explicit timestamps, we can take advantage of tools which help correct for gaps in an appropriate manner. 

### 4.1. Submit in Preferred Format with a Mapping File

Submissions should either be formatted as row-based or stream-based:
- Row-based: one sample and timestamp per row
...
- `sampling_rate`: the nominal sampling rate for the channel, in Hz (float) 
- `units`: a string giving the units of the channel

Row-based submissions should also have:
...
Submit your mapping file(s) to [CHoRUS to WFDB converter repo](https://github.com/chorus-ai/chorus_waveform_conversion). Provide all code under `mappings/<site>/` 

---

## 5. Linking Waveform Sessions to Visits

A table named `waveform_visit_links.csv` should be provided under the `/OMOP/` folder. This table should have a column for `visit_occurrence_id`, and `session_id`. If the data collection system does not provide a direct linkage between `session_id` and `visit_occurrence_id`, it may be necessary to infer this linkage based on the start and end time of the session. Keep in mind that the session start and end may not exactly match the admission, discharge, or transfer time in the EHR.
