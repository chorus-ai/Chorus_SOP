...
### 3.3. Stadardized channel naming

We expect the signal labels for each channel to follow the ISO/IEEE 11073 standards. The signal labels for requested channels can be found here:
- <wiki_telemetry_link>
- <wiki_eeg_link>
...
The exact sample rate (e.g. 249.97 Hz) needs to be passed when saving files in WFDB to ensure accurate timestamps. The device manufacturer should be able to provide the sample rate. While this may not be perfectly accurate, it is good enough initially.

### 5.6. Low Frequency Signals ("Numerics")

Low frequency signals (e.g. heart rate) should be saved as a CSV file in the same folder as the high frequency waveforms. The CSV file should be named as `<session_number>n.csv`. Please do not resample the low frequency signals in an effort to  create a uniform time interval. Please note that dates in numerics data must be shifted in manner consistent with the other electronic health record and waveform data being submitted, as described in the Time Shifting section above.

---
## 6. Submitting Waveforms

We encourage submitting waveforms in the "raw" format that you get as an output from your monitor. When waveform data is submitted with explicit timestamps we can take advantage of tools which help correct for gaps in an appropriate manner. 

In your SUBMISSION.md file, please indiacate which of the below options you took when submitting your data:

### 6.1. Submit in "raw" format with a mapping file

Please describe the "raw" format and include these details as appropriate:
...
Also, please submit a mapping file as requested in the next section.

#### 6.1.1. Mapping file requirements

Alasdair TO SUPPLY CONTENT

### 6.2. Submit in WFDB format

If you do not have access to your waveform data in a "raw" format with explicit timestamps, you can submit it in WFDB format. Ideally your WFDB files would have been converted to WFDB by using the CHoRUS to WFDB converter. When submitting in WFDB please indicate if the conversion was done with the CHoRUS to WFDB converter. If it was not done with this official converter please indicate:

- How you handled splitting data in to segments. Also, please indicate if you limited your maximum segment length to 8 hours as suggested above.
- How you handled gaps. Please indicate whether you were able to differentiate between gaps due to missing data and other types of gaps (e.g. a gap due to a clock resync). Please indicate how you modified your data for each gap type. 

## 6. Preparing Waveforms in an Alternative Format

As discussed in the SOP on [contributing waveforms](/Chorus_SOP/docs/Waveform-Data/), the WFDB format is strongly preferred. If providing an alternative format, detailed documentation about the format is required to facilitate conversion to WFDB. The documentation (in `SUBMISSION.md`) should include:
...
## 7. Linking Waveform Sessions to Visits

A table named `waveform_visit_links.csv` should be provided under the `/OMOP/` folder. This table should have a column for `visit_occurrence_id`, and `session`. If the data collection system does not provide a direct linkage between session and visit_occurrence_id, it may be necessary to infer this linkage based on the start and end time of the session. Keep in mind that the session start and end may not exactly match the admission, discharge, or transfer time in the EHR.
