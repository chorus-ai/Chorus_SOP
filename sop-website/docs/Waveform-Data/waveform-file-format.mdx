...
We expect the signal label to follow the ISO/IEEE 11073 standards. The signal labels for
channels that required or recommended can be found here (LINK).
...
A session (or record in WFDB terminology) represents a single "continous" recording from a patient. It is possible for a patient to have multiple sessions within a given hospital visit. We suggest making use of unique identifiers or bed numbers to separate waveform recordings. ADD EXAMPLE?? If there are no unique indentifiers for the recording in the data stream and bed numbers are not available, it is acceptable to provide all waveform data for a given patient's visit under a single session.  
...
A table named `waveform_visit_links.csv` should be provided under the `/OMOP/` folder. This table should have a column for `visit_occurrence_id`, and `session`. If an identifier is availabe which links a given waveform session to a visit then it should be used to generate this table. If such an identifier is not available, the waveform should be linked to a visit based on timestamps. If the start time of a waveform occurs between the start and end of a visit it should be linked to that visit. HOW TO TREAT EDGE CASES?
