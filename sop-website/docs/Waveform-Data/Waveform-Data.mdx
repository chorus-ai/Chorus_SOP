...
Below we list some of the waveform signals that should be submitted. A more complete list along with the standardized **signal label** for each telemetry signal being requested, can be found here: [https://github.com/chorus-ai/chorus_waveform_conversion/wiki/Waveform-Conversion](https://github.com/chorus-ai/chorus_waveform_conversion/wiki/Waveform-Conversion).
...
Waveform data should be submitted in your preferred format with minimal modification. This data should be kept as close to the "raw" format from your monitors as possible. However, you should deidentify it in alignment with your institutions policies for CHoRUS submissions. A mapping file also needs to be submitted.
...
2. **Submission Overview:**
   - Provide a text file named `SUBMISSION.md` (template available [here](https://raw.githubusercontent.com/chorus-ai/chorus_waveform_conversion/refs/heads/main/submission/templates/SUBMISSION_template.md)) under your `/OMOP/` folder, with the following high-level details:
     - Hospital name
     - Brief description of the dataset (number of patients, date range covered by waveforms, the types of waveform files being submitted and the number of each type)
